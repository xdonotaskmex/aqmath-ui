...
**No parameters of the adaptive engine were tuned for this comparison.** The adaptive threshold engine uses the same configuration validated on the production path before this study. The fixed-threshold baseline uses the Aegis (v14) production defaults that were in force when this study ran.

**Status since publication:** the adaptive engine this study favours, Proteus (v18), is now the production engine. Aegis (v14) remains only as a warm-up fallback for portfolios with too little history to calibrate an adaptive threshold. Every headline figure elsewhere on this site that predates that switch was measured on Aegis (v14), not on the engine running today.
