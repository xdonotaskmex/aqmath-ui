# 样本外验证 — Aegis（v14.0）去杠杆调节器在未见过的代币组合上的表现

**日期：** 2026-07-17
**引擎：** Aegis（v14.0）回测引擎（生产环境 `default_config`）
**结论：** ✅ 通过 — Aegis（v14）在其设计目标上具备样本外泛化能力
...
检验**已上线的生产配置**（Aegis v14.0）在**从未用于设计或调参的代币**上是否依然稳健。
这是一次真正的**资产选择样本外测试**：以下所有资产都不属于 Aegis（v14）的调参/验证组合。
...
**核心结论：** Aegis（v14.0）兑现了它的使命——在从未见过的代币（包括一个已归零的代币）上
实现大幅、稳定的回撤削减与更优的风险调整后收益。**建议：保持 Aegis（v14）不变。**
