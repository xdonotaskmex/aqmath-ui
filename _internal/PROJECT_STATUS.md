...
| **`/backtest` drawdown basis equalised** | engine | 2026-09-08 | Both engine legs now read the same window-local basket drawdown curve. Previously the v14 leg was handed a per-token full-history basis that v18 cannot be given and that no live path uses, which pinned it defensive on 3141/3184 days at 8% CAGR. Live-verified in production. Recorded honestly: the fix *reduced* v18's apparent backtest-tab advantage from Calmar 1.28 vs 0.53 to 1.28 vs 1.25 |
| **§8.5/§8.6 re-scoped onto the production path** | ui | 2026-09-08 | The continuous 8.7-year headline was retired — it does not reproduce and ran a retired v14 preset. Two sub-claims corrected *against us*: the sealed-half Sharpe "0.48 vs 0.37" was attributed backwards (0.48 is Buy & Hold's; the shield's is 0.24 vs B&H 0.30 on the survivors' window), and the dead-token Buy & Hold deterioration is +7.5pp, not "~18". Touches `en.json` + `zh-CN.json` + `_src` baked fallbacks + the hardcoded hero terminal strings in `app-boot.js` + `/backtest` meta & JSON-LD in `build_pages.py` |
| ⚠️ **Re-measure the remaining public evidence on v18** | engine | open | OOS-182, MC-3000, graveyard and liveness studies are still v14 output. Labelled as such, but not yet re-run on the live engine. Pre-launch blocker. The 8.7y headline is no longer on this list — it was retired and replaced rather than re-measured |
...
- ✅ **CLOSED 2026-09-08 (Railway dashboard, operator action):** the public `*.up.railway.app` domains were removed from all four API services, so the Cloudflare WAF can no longer be bypassed by calling the origin directly. Verified live: all four Railway origins now return **404** on `/` *and* on their operator paths, while the public `api-*.aqmath.xyz` hosts still return 200 with healthy payloads. The four origin hostnames and the exact operator-path list are deliberately not written here — see the `_internal/README.md` deny-list; they remain in this repo's git history, so treat them as known-but-dead
