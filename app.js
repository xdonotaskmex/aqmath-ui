...
// 404.html stores the originally requested path before bouncing to /.
try {
    const savedPath = sessionStorage.getItem('aq-path');
    if (savedPath) {
        sessionStorage.removeItem('aq-path');
        history.replaceState(null, '', savedPath);
    }
} catch (e) { /* sessionStorage unavailable — ignore */ }

const ROUTES = {
    '/':         { cls: 'route-landing',  title: 'AQMath — Crypto Portfolio Rebalancer | Risk Parity' },
    '/app':      { cls: 'route-app',      title: 'App — AQMath Portfolio Rebalancer' },
    '/docs':     { cls: 'route-doc',      title: 'System Documentation — AQMath' },
    '/backtest': { cls: 'route-backtest', title: 'Backtest — AQMath Deleverage Shield' },
    '/results':  { cls: 'route-results',  title: 'New-Token Stress Test — AQMath' }
};

function currentPath() {
    // Legacy hash routes (#/docs) → clean URL, keeps old links/bookmarks working
    const hash = window.location.hash;
    if (hash.indexOf('#/') === 0) {
        let p = '/' + hash.slice(2).replace(/\/+$/, '');
        if (p === '/doc') p = '/docs';
        p = ROUTES[p] ? p : '/';
...
function handleRoute() {
    ...
    if (canonical) canonical.setAttribute('href', 'https://aqmath.xyz' + (path === '/' ? '/' : path));
    if (path === '/' || path === '/app') {
        // Load market widgets for landing + app views
        loadAllWidgets();
    }
}
...
// Intercept internal route links (incl. i18n-injected ones) for SPA navigation
...
async function optimizePortfolio() {
    ...
    try {
        ...
        // weight 0 = optimizer explicitly excluded the token (KKT floor/cap) ->
        // target must be zeroed too, otherwise the stale target survives and
