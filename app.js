...
function initDocsChart() {
    if (docsChartInstance) return;
    const canvas = document.getElementById('docShieldChart');
    if (!canvas) return;
    if (typeof Chart === 'undefined') return;

    // Real engine-generated equity curves from backtest simulation
    // 3 tokens (ADA, BTC, XRP), 3184 days, sampled every 30 days
    // Generated by gen_chart_data.py using the actual evaluateShield() engine
    // DL: $8,431 final | B&H: $3,854 final | Both invested: $1,160
    const dlEquity = [100,116,167,211,242,285,319,367,423,395,440,468,581,533,524,441,433,440,442,449,456,469,489,521,509,443,463,455,439,416,502,584,559,578,745,1017,1115,1145,1000,850,851,863,899,904,1119,1288,1257,1226,1548,2035,2219,2164,2204,1825,1817,1674,1687,1615,1563,1530,1512,1527,1530,1515,1510,1548,1954,1808,1720,1950,2255,2106,2157,2057,2061,2043,2114,2057,2761,3108,3317,3253,4122,3936,3954,4451,4567,4852,5689,5704,5917,5505,5835,5360,5949,6548,7299,7674,6793,7424,7615,7975,8280,8357,8057,8053,8308,8431];
    const bhEquity = [100,117,169,214,245,289,323,371,428,400,445,473,587,488,497,372,180,167,129,123,117,141,158,183,175,147,164,163,153,133,177,213,210,223,294,408,453,471,417,335,289,308,378,390,511,594,584,576,732,967,1060,1047,1069,916,858,888,962,906,725,610,549,588,547,556,482,514,681,637,637,788,918,863,890,854,862,860,896,877,1183,1338,1434,1411,1794,1719,1732,1956,2012,2143,2519,2531,2631,2454,2606,2399,2669,2943,3286,3460,3069,3359,3451,3619,3763,3804,3673,3676,3798,3854];

    // Generate date labels (sampled every ~32 days over 8.7 years)
    const labels = [];
    const startDate = new Date(2017, 0, 1);
    const maxLen = Math.max(dlEquity.length, bhEquity.length);
    ...
    docsChartInstance = new Chart(canvas, {
        ...
        data: {
            ...
            datasets: [
                {
                    label: 'Deleverage + Tranche DCA ($8,431)',
                    data: dlEquity,
                    borderColor: '#06b6d4',
