...
const ROUTES = {
    '/':         { cls: 'route-landing',  view: 'landingView',   title: 'AQMath — Crypto Portfolio Rebalancer | Risk Parity' },
...
// Entry pages generated by tools/build_pages.py ship exactly one view.
function routeViewPresent(path) {
    const route = ROUTES[path];
    return !!(route && document.getElementById(route.view));
}
...
function currentPath() {
    ...
    // The generated pages are also reachable with their extension (/docs.html)
    // and via file:// previews — normalise both onto the clean route.
    p = p.replace(/\.html$/, '');
    p = p.slice(p.lastIndexOf('/'));
    if (p === '/index' || p === '') p = '/';
    if (p === '/doc') p = '/docs';
    if (routeViewPresent(p)) return p;
    // Unknown path: fall back to whichever view this document does ship.
    return Object.keys(ROUTES).find(routeViewPresent) || '/';
}
...
// Only routes whose view actually exists in THIS document can be handled in-page;
// anything else falls through to a normal navigation to the generated entry page.
document.addEventListener('click', function(e) {
    const a = e.target.closest && e.target.closest('a[href^="/"]');
    if (!a || a.target === '_blank') return;
    const path = a.getAttribute('href');
    if (ROUTES[path] && routeViewPresent(path)) {
        e.preventDefault();
        navigate(path);
    }
});
...
let editMode = null;
// Entry pages built by tools/build_pages.py ship ONLY their own view, so the
// app canvases are absent on /docs, /backtest and /results. Resolving them
// lazily keeps this file from dying at parse-time and taking the router with it.
const chartEl = document.getElementById('chart');
const historyEl = document.getElementById('historyChart');
const chartCtx = chartEl ? chartEl.getContext('2d') : null;
const historyCtx = historyEl ? historyEl.getContext('2d') : null;
const hasAppView = !!chartCtx;
if (window.Chart) {
...
(async () => {
    loadHistory();
    handleRoute();
    // Everything below writes into #appView. On the generated /docs, /backtest
    // and /results pages that view is not in the DOM, so stop here rather than
    // throwing on the first missing element.
    if (!hasAppView) return;
