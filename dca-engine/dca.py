...
async def distribute_dca(
    ...
    for t in active:
        ...
        # FIX: Include self-sustaining tokens (APY >= 10%) — they still need DCA if underweight.
        # The selfSustaining flag influences safety_factor (via volatility) but NOT eligibility.
        if stats["delta"] > 0.01 and not skip_due_to_trend:
            eligible.append({
                "token": t,
                "delta": stats["delta"],
    ...
    if not eligible:
        logger.info("[DCA] No eligible tokens after filtering (all at target/above trend)")
        return {
            "buy_summary": [],
            "warnings": ["No positions are below target allocation or all are above trend average. DCA not needed."],
