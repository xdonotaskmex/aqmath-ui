"""Kraken Collector - FastAPI application.

Collects historical crypto prices from Kraken API.
...
from collector import KrakenCollector
...
async def lifespan(app: FastAPI):
    ...
    # Schedule daily collection at 00:10 UTC (offset from coingecko)
    scheduler.add_job(daily_collection, "cron", hour=0, minute=10, timezone="UTC")
    scheduler.start()
    logger.info("Scheduler started. Daily collection at 00:10 UTC.")
...
app = FastAPI(
    title="Kraken Collector",
    description="Collects 365-day historical crypto prices from Kraken API. Forwards raw JSON to data-pipeline.",
    version="1.0.0",
    lifespan=lifespan,
)
...
@app.get("/")
async def root():
    return {"service": "kraken-collector", "status": "running", "db_access": False}
...
async def list_pairs():
    ...
    from config import SYMBOL_TO_KRAKEN_PAIR
    return {
        "total": len(SYMBOL_TO_KRAKEN_PAIR),
        "mappings": SYMBOL_TO_KRAKEN_PAIR,
    }
